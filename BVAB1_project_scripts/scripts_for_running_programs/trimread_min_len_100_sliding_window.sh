...
OUTPUTFOLDER="/Groups/assembly/BVAB1_PROJECT/TrimmomaticOutputMinLen100SlidingWindow10Phred30/"
...
for i in ${S[@]}
	do
		...
		java -jar /Groups/assembly/bin/Trimmomatic-0.30/trimmomatic-0.30.jar PE -threads 4 -phred33 -trimlog $OUTPUTFOLDER$i/$i.trimmomatic.trimlog $SOURCEFOLDER$i$SOURCEMID$ONE$SOURCEEND $SOURCEFOLDER$i$SOURCEMID$TWO$SOURCEEND $OUTPUTFOLDER$i/$i.trimmed.paired.1.fastq $OUTPUTFOLDER$i/$i.trimmed.unpaired.1.fastq $OUTPUTFOLDER$i/$i.trimmed.paired.2.fastq $OUTPUTFOLDER$i/$i.trimmed.unpaired.2.fastq LEADING:30 TRAILING:30 MINLEN:100 SLIDINGWINDOW:10:30
	done