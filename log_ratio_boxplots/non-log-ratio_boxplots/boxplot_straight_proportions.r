...
# this finds the sum of OTU abundance in selected samples, and divides by number of samples if weighted.
getOTUsums <- function(data,samples,pattern,weighted) {
	...
	if (weighted) {
		return(apply(data[indices],1,function(x) sum(x)/length(indices)))
	}
	return(apply(data[indices],1,function(x) sum(x)))
}
...
pdf("boxplots_straight_proportions.pdf")
